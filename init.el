...
;;; Configure Emacs to run following initialization code after it
;;; completes early initialization.
(add-hook 'after-init-hook
		  #'(lambda ()
			  ...
			  (when nil
				;; This is causing problems on Prometheus when flycheck runs
				;; on elisp buffers, complaining about no such file or
				;; directory.
				(setenv "TMPDIR"
...
(custom-set-variables
 ...
 '(org-todo-keywords
   '((sequence "MAYBE (m)" "TODO(t)" "STARTED(s)" "WAITING(w)" "PR(p)" "|" "MERGED(g)" "DONE(d)" "CANCELLED(c)" "DELEGATED(g)")))
